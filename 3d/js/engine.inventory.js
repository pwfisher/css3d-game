(function (w, d, GAME) {
  ...
  GAME.INV.RemoveItem = function (index) {
    ...
    if (
      GAME.INV.active_item &&
      items[index] === GAME.INV.active_item.parentNode
    ) {
      GAME.INV.active_item = null;
      d.removeEventListener("mousemove", mousemove);
      inv_preview.style.display = "none";
  ...
  GAME.INV.Render = function () {
    ...
    for (var i = 0; i < slots.length; ++i) {
      if (!slots[i]) items[i].innerHTML = "";
        ...
        var xtr = "";
        if (!GAME.UTILS.Mobile && slots[i].desc)
          xtr = 'title="' + slots[i].desc + '"';
        if (slots[i].use) xtr += ' data-use="' + slots[i].use + '"';

        items[i].innerHTML =
          '<img ondragend="GAME.INV.DragEnd(this)" ' +
          xtr +
  ...
  GAME.INV.DragEnd = function (el) {
    var element = document.elementFromPoint(
      drag_x - window.scrollX,
      drag_y - window.scrollY
    );
    if (!element) return;
