(function (w, d) {
  ...
  GAME.On = function (event_name, callback) {
    if (!_events[event_name]) _events[event_name] = [callback];
    else _events[event_name].unshift(callback);
  };
  ...
  GAME.On("ReqLoadLevel", function (level, objects) {
    ...
    // actual preloading
    var empty_pixel =
      "data:image/jpeg;base64,/9j/4AAQSkZJRgABAQAAAQABAAD/2wBDABsSFBcUERsXFhceHBsgKEIrKCUlKFE6P" +
    ...
    var total = level.PRELOAD.length,
      sofar = 0;
  ...
  GAME.OnFrame = function (frame_rate, repeat_times, callback) {
    ...
    var func = function (e) {
      ...
      if (gap >= time_per_frame) {
        ...
        if (callback) {
          skip = callback(
            orig_times - repeat_times,
            total >> 0,
            gap / (time_per_frame + remove)
          ); // gap between the calls
        }
  ...
  (function (w, d, GAME) {
    ...
    GAME.ACTIONS.DialogAsk = function (type, index, sub_index, callback) {
      ...
      if (sub_index !== undefined && sub_index / 1 === sub_index)
        data = data[sub_index];
      ...
      if (data.type === "choice") {
        ...
        var html =
          '<div class="' + clss + '"><h5>' + data.text + "</h5></div><br/>";
    ...
    GAME.ACTIONS.NextText = function (e) {
      ...
      if (active_txt.anim) {
        // active_txt.el && (active_txt.el.className !== '' && active_txt.el.className !== 'caret'))
        if (typeof entry === "string") {
      ...
      for (var i = 0; i < spans.length; ++i) spans[i].className = "old";
    ...
    GAME.ACTIONS.SetActiveText = function (type, index, clear_all, force_clss) {
      ...
      w.requestAnimationFrame(function () {
        ...
        if (clear_all)
          txt.innerHTML =
            '<div><div class="inl-bl"><span class="' +
            clss +
            '">' +
            act_text +
            "</span></div></div>";
    ...
    function __make_body(
      ...
      html =
        '<div><div class="inl-bl"><span onanimationend="GAME.ACTIONS.nextLine(this,' +
        line_delay +
        ')" class="' +
        clss +
        extra_class +
      ...
      if (append) txt.innerHTML += html;
      else txt.innerHTML = html;
