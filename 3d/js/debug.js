...
window.makeCube = function (width, height, length) {
  ...
  parent.style.cssText =
    "transform-style:preserve-3d;z-index:99999;transform:" +
    parent_trans +
...
window.makeTube = function (diameter, height, faces_num) {
  ...
  parent.style.cssText =
    "transform-style:preserve-3d;z-index:99999;transform:translate3d(0,0,100px);width: 2px; height: 2px; background: blue;";
  // 100px Z hack so that it is visible
  ...
  for (var c = 0; c < faces_num; c++) {
    ...
    parent.appendChild(
      createFace(side_len + 1, height, x, 0, z, 0, ry, 0, null, side_len * c, 0)
    );
  }
  //---
...
window.DrawPoint = function (vert) {
  ...
  el.id = id;
  el.style.transform =
    "translate3d(" + vert.x + "px," + vert.y + "px," + vert.z + "px)";
...
*/
  ...
  var transform = getComputedStyle(node.el)
