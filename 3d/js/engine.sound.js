  ...
  GAME.Sound = {
    ...
    PlayStep: function () {
      ...
      if (steps_extra && Math.random() > steps_extra.chance) {
        ...
        while (l-- > 0) {
          if (
            rand > steps_extra.sounds[l].chance &&
            prev_step !== steps_extra.sounds[l].sound
          ) {
    ...
    Play: function (sound_id, distance, interupt) {
      ...
      if (el.readyState >= 2) {
        ...
        if (distance) el.volume = (default_volume * distance).toFixed(2) / 1;
        else if (el.volume !== default_volume) el.volume = default_volume;
