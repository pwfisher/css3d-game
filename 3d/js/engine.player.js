(function (w, d, GAME) {
  ...
  GAME.On("ReqSetPersp", function (value, value2) {
    ...
    if (value2) {
      camera_degx = value2 > 99999 ? camera_degx + 0.01 : value2;
    }
  });
  ...
  GAME.On("ReqSetRot", function (rotation, confs) {
    ...
    if (steps > 0) {
      ...
      var ease_linear = function (t) {
        return t;
      };
      var ease_in_quad = function (t) {
        return t * t;
      };
      var ease_out_quad = function (t) {
        return t * (2 - t);
      };
      var ease_inout_quad = function (t) {
        return t < 0.5 ? 2 * t * t : -1 + (4 - 2 * t) * t;
      };
      var ease_in_elastic = function (t) {
        return (0.04 - 0.04 / t) * Math.sin(25 * t) + 1;
      };
      var ease_out_elastic = function (t) {
        return ((0.04 * t) / --t) * Math.sin(25 * t);
      };
      var ease_inout_elastic = function (t) {
        return (t -= 0.5) < 0
  ...
  function _getDims() {
    ...
    GAME.DIMS = {
      ...
      width: width,
      height: height,
      pos: new Vector(-move_x, 0, -move_z),
      deg: new Vector(camera_degy, camera_degx, 0),
    };
  }
  _getDims();
...
*/
  ...
  document
    .getElementById("scn-cntr")
    .addEventListener("touchmove", freezeVp, { passive: false });
  document
    .getElementById("ui")
    .addEventListener("touchmove", freezeVp, { passive: false });
  ...
  if (GAME.UTILS.Mobile) {
    ...
    (function (w, d, GAME) {
      ...
      mobile_controls.addEventListener("touchmove", freezeVp, {
        passive: false,
      });
      ...
      mobile_controls.addEventListener(
        "touchstart",
        function (e) {
          if (!GAME.State.control) return;
...
*/
  ...
  function closestSide(move_x, move_y, trigg) {
    ...
    var v2 = new Vector(
      (trigg.vertices[0].x + trigg.vertices[1].x) / 2,
      0,
      (trigg.vertices[0].z + trigg.vertices[1].z) / 2
    );
